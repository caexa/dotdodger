...
const slowFactor = 0.7; // Adjust slow speed multiplier here (0.7 is 70% speed)
...
document.addEventListener("keydown", e => {
  ...
  if (e.key === " " && player.dashCooldown <= 0) {
    ...
    // Clamp player.x inside canvas after dash
...
function drawCooldownBar() {
  if (player.dashCooldown > 0) {
    ...
    ctx.fillStyle = slowTime > 0 ? "cyan" : "cyan"; // always cyan here, but can change if wanted
    ctx.fillRect(300, 10, 80 * (1 - player.dashCooldown / 300), 10);
    drawText("Dash", 300, 35, "12px", slowTime > 0 ? "cyan" : "white");
  }
}
...
function update() {
  ...
  // Background tint for Slow Mode
  if (slowTime > 0) {
    ctx.fillStyle = "rgba(0, 0, 255, 0.1)";
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    drawText("SLOW MODE", 140, 90, "20px", "cyan");
  }
  ...
  for (let i = powerUps.length - 1; i >= 0; i--) {
    const p = powerUps[i];
    p.y += p.speed * (slowTime > 0 ? slowFactor : 1);
    ctx.beginPath();
  ...
  for (let i = dots.length - 1; i >= 0; i--) {
    const d = dots[i];
    d.y += slowTime > 0 ? d.speed * slowFactor : d.speed;
    ctx.beginPath();
  ...
  // Draw UI elements with color shift if slow mode active
  drawText(`Score: ${score}`, 10, 30, "20px", slowTime > 0 ? "cyan" : "white");
  if (player.hasShield) drawText("Shield", 10, 50, "16px", slowTime > 0 ? "cyan" : "cyan");
  drawCooldownBar();
